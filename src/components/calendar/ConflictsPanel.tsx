          ...
          <div key={i} style={{ background: "var(--surface)", borderRadius: 6, padding: 10, marginBottom: 8, border: "1px solid var(--danger)" }}>
            ...
            {c.appointment ? (
              <div style={{ fontSize: 12, color: "var(--text-muted)" }}>
                Has an appointment during the{" "}
                {formatTime(c.shifts[0].start_time)}–{formatTime(c.shifts[0].end_time)} shift
                {c.shifts[0].template && <span style={{ opacity: 0.6 }}> ({c.shifts[0].template.name})</span>}
              </div>
            ) : (
              c.shifts.map((s) => {
